...
**Candidate status:** `R12_HUMAN_VISUAL_REMEDIATION_INTEGRATED_VALIDATION_PENDING`  
**Candidate path:** `6-presentation-r12`  
**Approval status:** `PENDING`  
**Generated:** `2026-08-11T04:44:45.533+02:00`  
...
**Change note:** r12 applies only the requested visual remediation to Slides 10, 11, 12, 14, 16, 17 and 18. Exactly ten permitted `deck/src` files differ from frozen r11; slide copy, claims, prices, references, IDs, the 40-minute timer, 20-slide inventory, phase boundaries and approval state remain unchanged. Integrated validation is pending; this candidate remains unapproved until an explicit in-tool human decision.
...
| 11 | S11 | `deck/src/slides/ModelPortfolioSlide.jsx` | Present the Luna/Terra/Sol escalation model while retaining the approved specialist foundation as governed metadata. | `P6-C015`–`P6-C018` |
...
| S11 | Use the smallest approved model that meets evidence needs. The visible Specialist Foundation section was removed, but `P6-C017` remains governed metadata and promotion remains benchmarked, human validated and fail closed. |
...
- Frozen-r11 baseline plus the three r12 slide-source contracts and r12 metadata contract: **PASS**.
- The r12 metadata contract proves exactly ten permitted `deck/src` differences and limits `deck.config.js` changes to revision and evidence-boundary metadata.
- Markdown and self-contained HTML differ from r11 only in r12 revision and validation metadata; the claim catalogue remains byte-identical.
- Phase 7 and Phase 8 remain `NOT_INVOKED`.
- No Phase 6 r12 approval is claimed by this candidate.

## Output paths

- `6-presentation-r12/stratton-presentation.md`
- `6-presentation-r12/stratton-presentation.html`
- `6-presentation-r12/stratton-claim-catalogue.json`
- `6-presentation-r12/evidence/stratton-aff-6-model-receipt.json`
- `6-presentation-r12/deck/deck.config.js`
- `6-presentation-r12/deck/src/`

## Gate boundary

This candidate records no approval. AFF-A, AFF-B, compliance coverage and human approval are
external lifecycle gates. Phase 6 r12 remains unapproved until the human architect records an
