...
const governanceControls = [
  { id: 'data-zone', icon: GlobeLock, label: 'EU Data Zone' },
  { id: 'versions', icon: LockKeyhole, label: 'Pinned versions' },
  { id: 'benchmark', icon: Gauge, label: 'Benchmark before promotion' },
  { id: 'validation', icon: UserRoundCheck, label: 'Human validation' },
]
  ...
  return (
    <Slide index={index} className={`${sharedStyles.phaseSlide} ${styles.modelPortfolioSlide}`}>
      ...
      <div className={`${sharedStyles.body} ${styles.body} content-frame content-gutter`}>
        ...
        <section className={`${sharedStyles.surface} ${styles.governanceRail}`}>
          <div className={styles.railLead}>
            <ShieldCheck aria-hidden="true" strokeWidth={1.65} />
            <div>
              <Editable as="strong" id="model-portfolio.governance.title">Promotion guardrails</Editable>
              <Editable as="span" id="model-portfolio.governance.subtitle" multiline>
                Every route remains governed and reversible
              </Editable>
            </div>
          </div>

          <EditableList
            id="model-portfolio.governance"
            items={governanceControls}
            keyOf={(item) => item.id}
            className={styles.governanceList}
            itemClassName={styles.governanceShell}
          >
            {(item) => {
              ...
              return (
                <div className={styles.governanceItem}>
                  ...
                  <Editable as="span" id={`model-portfolio.governance.${item.id}`}>
                    {item.label}
                  </Editable>
                </div>
              )
