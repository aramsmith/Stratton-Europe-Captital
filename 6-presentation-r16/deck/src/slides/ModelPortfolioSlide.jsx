...
const aiTechniques = [
  { id: 'prompt-engineering', icon: null, label: 'Prompt Engineering', diagram: 'prompt' },
  { id: 'loop-engineering', icon: null, label: 'Loop Engineering', diagram: 'loop' },
  {
...
function PromptGraph() {
  return (
    <svg
      className={styles.promptGraph}
      viewBox="0 0 114 50"
      role="img"
      aria-label="A central prompt connects to five prompt engineering techniques"
      data-prompt-graph
    >
      <g className={styles.promptLinks}>
        <path d="M 41 21 L 15 10" />
        <path d="M 41 29 L 15 40" />
        <path d="M 73 21 L 99 10" />
        <path d="M 73 29 L 99 40" />
        <path d="M 57 33 L 57 43" />
      </g>

      <rect className={styles.promptCore} x="41" y="17" width="32" height="16" rx="4" />
      <path className={styles.promptGlyph} d="M 47 22 L 51 25 L 47 28 M 56 28 L 65 28" />

      <circle className={`${styles.promptNode} ${styles.promptSpecificity}`} cx="10" cy="8" r="5" />
      <circle className={`${styles.promptNode} ${styles.promptContext}`} cx="10" cy="42" r="5" />
      <circle className={`${styles.promptNode} ${styles.promptConstraints}`} cx="104" cy="8" r="5" />
      <circle className={`${styles.promptNode} ${styles.promptRefinement}`} cx="104" cy="42" r="5" />
      <circle className={`${styles.promptNode} ${styles.promptReasoning}`} cx="57" cy="46" r="4" />
    </svg>
  )
}
...
function LoopGraph() {
  return (
    <svg
      className={styles.loopGraph}
      viewBox="0 0 114 50"
      role="img"
      aria-label="Work enters a decision, loops back when repeated, and exits to done"
      data-loop-graph
    >
      <defs>
        <marker
          id="slide11-loop-forward-arrow"
          viewBox="0 0 6 6"
          refX="5"
          refY="3"
          markerWidth="5"
          ...
          orient="auto"
        >
          <path d="M 0 0 L 6 3 L 0 6 Z" className={styles.loopForwardArrow} />
        </marker>
        <marker
          id="slide11-loop-return-arrow"
          viewBox="0 0 6 6"
          refX="5"
          refY="3"
          markerWidth="5"
          ...
          orient="auto"
        >
          <path d="M 0 0 L 6 3 L 0 6 Z" className={styles.loopReturnArrow} />
        </marker>
      </defs>

      <g className={styles.loopForwardLinks}>
        <path d="M 16 29 L 39 29" markerEnd="url(#slide11-loop-forward-arrow)" />
        <path d="M 61 29 L 91 29" markerEnd="url(#slide11-loop-forward-arrow)" />
      </g>
      <path
        className={styles.loopReturnLink}
        d="M 50 18 C 46 3, 27 3, 27 17"
        markerEnd="url(#slide11-loop-return-arrow)"
      />

      <circle className={styles.loopWorkNode} cx="9" cy="29" r="7" />
      <path className={styles.loopDecisionNode} d="M 50 18 L 61 29 L 50 40 L 39 29 Z" />
      <rect className={styles.loopDoneNode} x="96" y="22" width="14" height="14" rx="2" />
    </svg>
  )
}
        ...
        <section className={`${sharedStyles.surface} ${styles.techniquesRail}`}>
          ...
          >
            {(item) => {
              ...
              const techniqueClass = item.diagram
                ? styles[`${item.diagram}TechniqueItem`]
                : ''
              return (
                <div
                  className={`${styles.techniqueItem} ${techniqueClass}`}
                >
                  {item.diagram === 'prompt' ? (
                    <PromptGraph />
                  ) : item.diagram === 'loop' ? (
