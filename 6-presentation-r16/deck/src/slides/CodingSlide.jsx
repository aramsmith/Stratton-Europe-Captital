...
const repoInventory = [
  { id: 'app', path: 'app/', count: '43', text: 'TypeScript API + worker' },
  { id: 'infra', path: 'infra/', count: '39', text: 'Tenant-scope Bicep + DU entrypoints' },
  { id: 'tests', path: 'tests/', count: '27', text: 'IaC + application verification' },
  { id: 'validation', path: 'validation/', count: '10', text: 'Fail-fast evidence scripts' },
  { id: 'delivery', path: 'deploy/ + support', count: '5', text: 'Release, tooling + root README' },
]
...
const skuRows = [
  ...
  },
  {
    ...
    status: 'Fixed/current evidence',
    note: 'Current Phase 5 package; Phase 3 Standard v2 remains an explicit upstream variance.',
  },
  {
    id: 'azure-openai',
    service: 'Azure OpenAI S0 · DataZoneStandard',
    tier: 'gpt-5.6-luna · gpt-5.6-terra · gpt-5.6-sol',
    status: 'Pinned/current evidence',
    note: 'Version 2026-07-09 · NoAutoUpgrade; IDs, quota and capacity remain owner inputs.',
  },
  {
  ...
  return (
    <Slide index={index} className={`${styles.phaseSlide} ${styles.codingSlide} ${slideStyles.codingSlide}`}>
      ...
      <div className={`${styles.body} content-frame content-gutter`}>
        ...
        <div className={`${styles.codingGrid} ${slideStyles.codingGrid}`}>
          <section className={`${styles.surface} ${styles.repoPanel} ${slideStyles.repoPanel}`}>
            <div className={styles.panelLead}>
              <div>
                ...
                <Editable as="h3" id="phase5-coding.repo.title">124 deployable source files</Editable>
              </div>
          ...
          <div className={`${styles.codingEvidence} ${slideStyles.codingEvidence}`}>
            <section className={`${styles.surface} ${styles.localEvidencePanel} ${slideStyles.localEvidencePanel}`}>
            ...
            <section className={`${styles.surface} ${styles.supplyChainPanel} ${slideStyles.supplyChainPanel}`}>
          ...
          <section className={[styles.surface, styles.skuPanel, slideStyles.skuPanel].join(' ')}>
            <div className={[styles.skuHeader, slideStyles.skuHeader].join(' ')}>
              <div>
            ...
            <EditableList
              ...
              items={skuRows}
              keyOf={(item) => item.id}
              className={[styles.skuRows, slideStyles.skuRows].join(' ')}
              itemClassName={[styles.skuRowShell, slideStyles.skuRowShell].join(' ')}
            >
              {(item) => (
                <article className={[styles.skuRow, slideStyles.skuRow].join(' ')}>
