...
**Candidate status:** `R14_SLIDE11_GRAPH_DIAMOND_VALIDATED`  
**Candidate path:** `6-presentation-r14`  
**Approval status:** `PENDING`  
**Generated:** `2026-08-21T11:11:29.537+02:00`  
...
**Change note:** r14 changes only the third Slide 11 `AI Techniques used` block requested by the human architect. `Graph Engineering - Chain` becomes `Graph Engineering - Diamond` and gains a compact one-to-three-to-one diamond graph illustration. Underlying governed model-portfolio controls remain authoritative metadata; all other slide copy, claims, prices, references, IDs, the 40-minute timer, 20-slide inventory, phase boundaries and approval state remain unchanged from frozen r13. Integrated validation passed; this candidate remains unapproved until an explicit in-tool human decision.
...
| S11 | Use the smallest approved model that meets evidence needs. The bottom rail identifies Prompt Engineering, Loop Engineering and Graph Engineering - Diamond, illustrated as one input branching across three paths before converging. `P6-C017` remains governed metadata and promotion remains benchmarked, human validated and fail closed. |
...
- Frozen-r13 baseline plus the focused r14 Slide 11 content contract: **PASS**.
- The r14 change is limited to `ModelPortfolioSlide.jsx`, its CSS module, revision metadata and the focused regression test.
...
- No Phase 6 r14 approval is claimed by this candidate.

## Output paths

- `6-presentation-r14/stratton-presentation.md`
- `6-presentation-r14/stratton-presentation.html`
- `6-presentation-r14/stratton-claim-catalogue.json`
- `6-presentation-r14/deck/deck.config.js`
- `6-presentation-r14/deck/src/`

## Gate boundary

This candidate records no approval. AFF-A, AFF-B, compliance coverage and human approval are
external lifecycle gates. Phase 6 r14 remains unapproved until the human architect records an
