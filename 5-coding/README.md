...
Phase 4 used `iac/...` as the logical release-package root. The Phase 5 output contract places that
package beneath `5-coding/`, so the path mapping is:

| Approved logical path | Phase 5 path |
|---|---|
| `iac/infra/` | `5-coding/infra/` |
| `iac/contracts/`, application and database work | `5-coding/app/` |
| `iac/tests/` | `5-coding/tests/` |
| `iac/validation/` | `5-coding/validation/` |
| release and deployment instructions | `5-coding/deploy/` and the root release manifest |
...
leave licence acceptance to AFF-B rather than silently accepting a policy. Final report generation
is followed by `New-Phase5Hashes.ps1` and
`Test-ReleaseEvidence.ps1` so reviewers receive one checkout-independent, self-excluding hash set.
The hash set includes the release manifest, both build reports and only the validation, container,
source-security and dependency evidence referenced by that release candidate; superseded local runs
remain audit history but are not part of the frozen candidate.
...
After the full local gate passes, freeze the candidate in this order:
