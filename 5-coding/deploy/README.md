...
1. Verify the approved `STRATTON-CC-001` change-control record, approved Phase 5 hash manifest and
   immutable Phase 5 approval.
...
4. Confirm every `REQUIRED_OWNER_INPUT` sentinel has been replaced by approved non-secret evidence.
5. Confirm all seven owner-bound controls are satisfied for the intended scope or remain outside it.
...
```powershell
pwsh -NoProfile -File .\5-coding\validation\Invoke-LocalValidation.ps1 -Scope All
```
...
$compiledOutput = @(
  ...
  --file ".\5-coding\infra\parameters\$Environment.bicepparam" `
  --stdout 2>&1 |
...
The human owner must now replace every sentinel in `$ApprovedParameters`. Do not edit the packaged
`.bicepparam` file. Bind the final file to the Phase 7 authorisation after selecting the exact target:
...
$preflight = @(
  '-NoProfile',
  '-File', (Join-Path $CaseRoot '5-coding\tests\iac\Invoke-DeploymentPreflight.ps1'),
...
Confirm required resource providers, quotas and
`Microsoft.Network/EnableApplicationGatewayNetworkIsolation` registration. For DU-01 through DU-15
and DU-17, the independently selectable root is a tenant-scope deployment:

```powershell
$Template = Join-Path $CaseRoot '5-coding\infra\main.bicep'
...
$AssuranceTemplate = Join-Path $CaseRoot '5-coding\infra\assurance-main.bicep'
