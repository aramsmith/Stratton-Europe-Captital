...
const risks = [
  { id: 'quality', mitigationId: 'profile', pairIndex: 0, title: 'Data quality and variation', text: 'Incomplete, inconsistent or changing portfolio formats can weaken extraction, comparison and reporting.' },
  { id: 'exposure', mitigationId: 'private', pairIndex: 1, title: 'Sensitive-data exposure', text: 'Incorrect access, residency or retention controls could expose confidential investment information.' },
  { id: 'grounding', mitigationId: 'evidence', pairIndex: 2, title: 'Ungrounded AI output', text: 'Weak provenance or citations could create unsupported conclusions or false confidence.' },
  { id: 'dependencies', mitigationId: 'fail-closed', pairIndex: 3, title: 'Dependency readiness', text: 'Identity, owner values, IPAM/DNS, quota, model and commercial decisions can delay implementation.' },
  { id: 'economics', mitigationId: 'poc', pairIndex: 4, title: 'Unproven economics', text: 'Performance, operating effort and Azure cost remain uncertain until tested with representative Stratton data.' },
]

const mitigations = [
  { id: 'profile', riskId: 'quality', pairIndex: 0, title: 'Profile before build', text: 'Profile the representative subset, document exceptions and agree a canonical POC mapping.' },
  { id: 'private', riskId: 'exposure', pairIndex: 1, title: 'Private and least privilege', text: 'Use private endpoints, managed identities, RBAC/PIM, approved retention and no model-training use.' },
  { id: 'evidence', riskId: 'grounding', pairIndex: 2, title: 'Evidence before decision', text: 'Require citations, provenance thresholds, evaluation evidence and retained human approval.' },
  { id: 'fail-closed', riskId: 'dependencies', pairIndex: 3, title: 'Fail closed on inputs', text: 'Block progress when accountable owners, dependencies or control evidence are missing.' },
  { id: 'poc', riskId: 'economics', pairIndex: 4, title: 'POC before scale', text: 'Measure quality, performance, operating effort and cost in non-production, then make an explicit scale decision.' },
]
              ...
              {(item) => (
                <>
                  <article
                    ...
                    style={{ '--pair-index': item.pairIndex }}
                    data-risk-pair={item.id}
                  >
                  ...
                  <span className={styles.pairConnector} aria-hidden="true" style={{ '--pair-index': item.pairIndex }}>
                    <span />
