...
const FONT_MODE_JETBRAINS = 'jetbrains'
const FONT_MODE_SYSTEM = 'system'
const FONT_STORAGE_KEY = 'stratton-r4-font-mode'
const FONT_MODES = new Set([FONT_MODE_JETBRAINS, FONT_MODE_SYSTEM])
...
function readStoredFontMode() {
  if (typeof window === 'undefined') return FONT_MODE_JETBRAINS

  try {
    const storedMode = window.localStorage.getItem(FONT_STORAGE_KEY)
    return FONT_MODES.has(storedMode) ? storedMode : FONT_MODE_JETBRAINS
  } catch (error) {
    console.warn('[Stratton deck] Unable to read the saved font preference.', error)
    return FONT_MODE_JETBRAINS
  }
}
  ...
  return (
    <div className={styles.status} aria-hidden="false">
      <button
        ...
        title={`${fontToggleLabel}. Current font: ${usingSystemFont ? 'system font' : 'JetBrains Mono'}.`}
        onClick={() => setFontMode(
          usingSystemFont ? FONT_MODE_JETBRAINS : FONT_MODE_SYSTEM,
        )}
      >
