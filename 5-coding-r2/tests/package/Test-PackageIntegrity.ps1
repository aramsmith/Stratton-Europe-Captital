...
$revision13Path = Join-Path (Split-Path -Parent $root) `
    '0-coordination\stratton-model-plan-revision-13.json'
$revision13Hash = (Get-FileHash -LiteralPath $revision13Path -Algorithm SHA256).Hash.ToLowerInvariant()
if ($revision13Hash -cne '9c18ca9fadea478470a2f23e7bd50a68b276b55f3fe9accdc299b82c7ad0284a') {
    throw 'Model-plan revision 13 hash mismatch.'
}
$revision13 = Get-Content -Raw -LiteralPath $revision13Path | ConvertFrom-Json -Depth 30
if (
    [string]$revision13.planRevision -cne '13' -or
    [string]$revision13.revisionedCandidate.packageRoot -cne '5-coding-r2' -or
    [string]$revision13.revisionedCandidate.manifestPath -cne
        '5-coding-r2/stratton-phase-5-hashes.json'
) {
    throw 'Model-plan revision 13 does not bind the revisioned candidate package.'
}
