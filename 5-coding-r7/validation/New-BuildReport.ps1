...
foreach ($azureBoundaryField in @(
    ...
    if ($release.azureExecution.$azureBoundaryField -ne $false) {
        throw "Build report generation refuses Azure activity: $azureBoundaryField"
    }
}
...
$validationRelativePath = [string]$evidenceSet.indexRelativePath
if (
    ...
    throw 'Release manifest and build report do not share the exact validation input binding.'
}
...
if (
    ...
    throw 'Release manifest and build report do not share the approved release-evidence boundary.'
}
...
$currentOwnerBoundGaps = @($release.currentOwnerBoundGaps)
$retainedMinorGaps = @($release.retainedMinorGaps)
...
$currentGapsMarkdown = if ($currentOwnerBoundGaps.Count -eq 0) {
    '- None recorded.'
}
else {
    ($currentOwnerBoundGaps | ForEach-Object { "- $_" }) -join "`n"
}
$minorGapsMarkdown = if ($retainedMinorGaps.Count -eq 0) {
    '- None recorded.'
}
else {
    ($retainedMinorGaps | ForEach-Object { "- ``$_``" }) -join "`n"
}
...
- Validation input SHA-256: **$($evidenceSet.validationInput.aggregateSha256)** ($($evidenceSet.validationInput.fileCount) files)
...
- Model-portfolio change control: ``$($release.modelPortfolioChangeControl.changeControlId)`` / ``$($release.modelPortfolioChangeControl.status)``
- CC-002 approval: ``$($release.modelPortfolioChangeControl.changeControlApproval.path)`` / ``$($release.modelPortfolioChangeControl.changeControlApproval.sha256)``
- Phase 3 CC-002 approval: ``$($release.modelPortfolioChangeControl.phase3Approval.path)`` / ``$($release.modelPortfolioChangeControl.phase3Approval.sha256)``
- Phase 4 CC-002 approval and subject: ``$($release.modelPortfolioChangeControl.phase4Approval.sha256)`` / ``$($release.modelPortfolioChangeControl.phase4Subject.sha256)``
- Superseded approved r4 manifest and approval: ``$($release.modelPortfolioChangeControl.supersededApprovedPhase5Baseline.manifest.sha256)`` / ``$($release.modelPortfolioChangeControl.supersededApprovedPhase5Baseline.approval.sha256)``
- Candidate root and canonical manifest: ``$($release.modelPortfolioChangeControl.candidate.packageRoot)`` / ``$($release.modelPortfolioChangeControl.candidate.canonicalManifestPath)``
- AFF-5 implementation author/model receipt: ``$($release.implementationReceipt.authorModel)``
...
$currentGapItems = if ($currentOwnerBoundGaps.Count -eq 0) {
    '<li>None recorded.</li>'
}
else {
    ($currentOwnerBoundGaps | ForEach-Object { "<li>$(ConvertTo-HtmlText -Value ([string]$_))</li>" }) -join "`n"
}
$minorGapItems = if ($retainedMinorGaps.Count -eq 0) {
    '<li>None recorded.</li>'
}
else {
    ($retainedMinorGaps | ForEach-Object { "<li><code>$(ConvertTo-HtmlText -Value ([string]$_))</code></li>" }) -join "`n"
}
  ...
  <main>
    ...
    <section class="grid">
      <article class="card">
        ...
        <p>Validation input: <code>$(ConvertTo-HtmlText -Value ([string]$evidenceSet.validationInput.aggregateSha256))</code> ($($evidenceSet.validationInput.fileCount) files)</p>
      ...
      <article class="card">
        ...
        <p>No Azure login, query, network call, target validation, what-if, deployment, inference, promotion, retention finalisation, production signing, or cloud runtime test was executed.</p>
        <p>The full validation release step covers the release manifest only. Build-report generation, canonical hashing and Test-ReleaseEvidence are the subsequent freeze sequence.</p>
      ...
      <article class="card">
        ...
        <p>Model portfolio change control: <code>$(ConvertTo-HtmlText -Value ([string]$release.modelPortfolioChangeControl.changeControlApproval.sha256))</code></p>
        <p>Phase 4 CC-002 subject: <code>$(ConvertTo-HtmlText -Value ([string]$release.modelPortfolioChangeControl.phase4Subject.sha256))</code></p>
        <p>Superseded approved r4: <code>$(ConvertTo-HtmlText -Value ([string]$release.modelPortfolioChangeControl.supersededApprovedPhase5Baseline.manifest.sha256))</code></p>
        <p>Candidate: <code>$(ConvertTo-HtmlText -Value ([string]$release.modelPortfolioChangeControl.candidate.canonicalManifestPath))</code></p>
        <p>AFF-5 author receipt: <code>$(ConvertTo-HtmlText -Value ([string]$release.implementationReceipt.authorModel))</code></p>
      ...
      <article class="card wide">
        ...
        <p>Final hash generation and independent release-evidence recomputation occur after this retained full validation run.</p>
      </article>
      ...
      <article class="card wide">
        <h2>Current owner-bound gaps</h2>
        <ul>$currentGapItems</ul>
      </article>

      <article class="card wide">
        <h2>Retained AFF-B minor gaps</h2>
        <ul>$minorGapItems</ul>
      </article>
      ...
      <article class="card wide">
        ...
        <ul>
          ...
          <li>Callers cannot select deployments or models; routing remains deterministic and application-owned.</li>
          <li>Data Zone Standard is required, Global Standard is rejected, and 2026-07-09 with NoAutoUpgrade is pinned.</li>
          <li>Production inference and benchmark promotion remain blocked; benchmark observations remain null.</li>
          <li>No autonomous decision authority is granted.</li>
          ...
          <li>Fourteen owner-bound controls and two retained AFF-B minor gaps remain open and unwaived.</li>
