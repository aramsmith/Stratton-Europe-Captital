...
$validationInputRelativePath = "evidence/local-validation/$runId/validation-input.json"
$validationInputManifest = New-ValidationInputManifest -PackageRoot $resolvedRoot `
    -RunId $runId -OutputPath (Join-Path $resolvedRoot $validationInputRelativePath)
$validationInputBinding = [ordered]@{
    manifest = $validationInputRelativePath
    aggregateAlgorithm = [string]$validationInputManifest.aggregateAlgorithm
    aggregateSha256 = [string]$validationInputManifest.aggregateSha256
    fileCount = [int]$validationInputManifest.fileCount
}
...
'@
    ...
    $requiredPaths = @(
        ...
        'validation\ValidationEvidence.psm1',
...
function Invoke-RecordedStep {
    ...
    $record = [ordered]@{
        ...
        validationInput = $validationInputBinding
    }
...
function Write-ValidationIndex {
    ...
    $index = [ordered]@{
        ...
        evidenceReferences = [ordered]@{
            ...
            sourceSecuritySummary = "evidence/source-security/$runId/summary.json"
            containerSummary = "evidence/containers/$runId/summary.json"
        }
        releaseEvidenceBoundary = [ordered]@{
            localValidationReleaseStep = 'RELEASE_MANIFEST_ONLY'
            finalFreezeOutsideValidationRun = $true
            freezeSequence = @(
                'validation/New-BuildReport.ps1',
                'validation/New-Phase5Hashes.ps1',
                ...
                -Action {
                    Invoke-NativeChecked -FilePath 'pwsh' `
                        -ArgumentList @(
                            '-NoProfile',
                            '-File',
                            (Join-Path $resolvedRoot 'validation\Capture-OciDigestEvidence.ps1'),
                            '-PackageRoot',
                            $resolvedRoot,
                            '-ValidationRunId',
                            $runId,
                            '-ValidationInputManifestPath',
                            $validationInputRelativePath
                        ) `
                        -WorkingDirectory $resolvedRoot
                }
        }
                ...
                -Action {
                    Invoke-NativeChecked -FilePath 'pwsh' `
                        -ArgumentList @(
                            '-NoProfile',
                            '-File',
                            (Join-Path $resolvedRoot 'validation\Scan-SourceSecurity.ps1'),
                            '-ValidationRunId',
                            $runId,
                            '-ValidationInputManifestPath',
                            $validationInputRelativePath
                ...
                -Action {
                    Invoke-NativeChecked -FilePath 'pwsh' `
                        -ArgumentList @(
                            '-NoProfile',
                            '-File',
                            (Join-Path $resolvedRoot 'validation\Build-ContainerEvidence.ps1'),
                            '-ValidationRunId',
                            $runId,
                            '-ValidationInputManifestPath',
                            $validationInputRelativePath
