...
param(
    ...
    [Parameter(Mandatory)][string]$ValidationInputManifestPath
)

$ErrorActionPreference = 'Stop'
$resolvedPackageRoot = (Resolve-Path -LiteralPath $PackageRoot).Path
if ([string]::IsNullOrWhiteSpace($BicepRoot)) {
    $BicepRoot = Join-Path $resolvedPackageRoot 'infra'
}
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $resolvedPackageRoot 'evidence\dependency-evidence.json'
}
$resolvedRoot = (Resolve-Path -LiteralPath $BicepRoot).Path
$validationEvidenceModule = Join-Path $resolvedPackageRoot 'validation\ValidationEvidence.psm1'
Import-Module -Name $validationEvidenceModule -Force -ErrorAction Stop
$verifiedInput = Test-ValidationInputManifest -PackageRoot $resolvedPackageRoot `
    -ManifestPath (Join-Path $resolvedPackageRoot $ValidationInputManifestPath) `
    -ExpectedRunId $ValidationRunId
$validationInput = [ordered]@{
    manifest = $ValidationInputManifestPath.Replace('\', '/')
