...
Describe 'Stratton Phase 5 remediation regressions' {
  BeforeAll {
    ...
    $packageRoot = (Resolve-Path (Join-Path $testsRoot '..\..')).Path
    ...
    $assuranceTemplate = Build-Template -Path (Join-Path $infraRoot 'modules\assurance\main.bicep')
    $retentionFinalizationScript = Join-Path $packageRoot 'deploy\Invoke-AssuranceRetentionFinalization.ps1'
  }
  ...
  It 'removes delivery write authority and scopes the controlled copier to the assurance evidence container' {
    $roleAssignments = @(
      $assuranceTemplate.resources |
        Where-Object { $_.type -eq 'Microsoft.Authorization/roleAssignments' }
    )
    $roleAssignments.Count | Should -Be 3
    (@($roleAssignments | ForEach-Object { [string]$_.properties.principalId }) -join "`n") |
      Should -Not -Match 'deliveryPrincipal'

    $copierAssignments = @(
      $roleAssignments |
        Where-Object {
          [string]$_.properties.principalId -match 'evidenceReaderCopierPrincipalId'
        }
    )
    $copierAssignments.Count | Should -Be 1
    [string]$copierAssignments[0].scope | Should -Match 'blobServices/containers'
    [string]$copierAssignments[0].scope | Should -Match 'evidenceStorage'
    [string]$copierAssignments[0].properties.roleDefinitionId |
      Should -Match 'evidenceReaderCopierRoleDefinitionId'
    $assuranceTemplate.outputs.PSObject.Properties.Name | Should -Contain 'retentionActivationState'
    $assuranceTemplate.outputs.PSObject.Properties.Name | Should -Contain 'dataAdmissionEnabled'
  }

  It 'keeps irreversible retention finalization behind explicit execution, authorisation and observed-state checks' {
    $raw = & pwsh -NoProfile -File $retentionFinalizationScript `
      -SubscriptionId '00000000-0000-4000-8000-000000000001' `
      -TenantId '00000000-0000-4000-8000-000000000002' `
      -ResourceGroupName 'rg-assurance' `
      -EvidenceStorageAccountName 'stevidence' `
      -EvidenceContainerName 'evidence' `
      -VerdictStorageAccountName 'stverdict' `
      -VerdictContainerName 'verdict' `
      -LegalHoldTags 'audit' `
      -HumanAuthorisationPath 'missing-authorisation.json' `
      -HumanAuthorisationSha256 ('0' * 64) `
      -EvidenceOutputDirectory $testWorkRoot 2>&1
    $text = ($raw | ForEach-Object { $_.ToString() } | Out-String)
    $LASTEXITCODE | Should -Not -Be 0
    $text | Should -Match 'requires -Execute'

    $scriptText = Get-Content -Raw -LiteralPath $retentionFinalizationScript
    $scriptText | Should -Match 'immutability-policy'', ''lock'
    $scriptText | Should -Match '''--if-match'', \$etag'
    $scriptText | Should -Match 'legal-hold'', ''set'
    $scriptText | Should -Match 'state -cne ''Locked'''
    $scriptText | Should -Match 'Observed legal hold is incomplete'
    ([regex]::Matches($scriptText, '''--subscription'', \$SubscriptionId')).Count |
