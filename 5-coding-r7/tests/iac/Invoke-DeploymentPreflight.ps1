...
function Test-StrictNonEmptyString {
  param($Value)
...
function Test-Sha256Hex {
  param($Value)
  return $Value -is [string] -and $Value -cmatch '\A[a-f0-9]{64}\z'
}

function Test-StrictEvidenceString {
  param($Value)
  return $Value -is [string] -and $Value -cmatch '\A[^\s\p{C}](?:[^\p{C}]*[^\s\p{C}])?\z'
}

function Test-ExactObjectKeys {
  param($Value,[string[]]$ExpectedKeys)
  if ($null -eq $Value -or $Value -is [string] -or $Value -is [System.Collections.IEnumerable]) {
    return $false
  }
  $actualKeys = @(Get-ObjectKeys $Value | Sort-Object)
  $expected = @($ExpectedKeys | Sort-Object)
  return $actualKeys.Count -eq $expected.Count -and (($actualKeys -join ',') -ceq ($expected -join ','))
}
...
$globalPrerequisitesByDu = @{
  ...
  'DU-11' = @('strattonNonproductionSubscriptionId','strattonProductionSubscriptionId','modelPortfolioDeploymentEnabled','modelNameVersionAndQuota','providerDataUseEvidenceId','aiActClassificationEvidenceId')
...
foreach($envName in @('dev','tst','prd')) {
  ...
  $deploymentSkuValues += $serviceType
  foreach($dep in To-Enumerable (Get-ObjectValue $params "du11.aiByEnvironment.$envName.deployments")) {
    $deploymentSkuValues += [string]$dep.skuName
  }
}
Add-Assertion 'ASR-04' 'Azure AI deployments use EU Data Zone Standard and reject Global Standard or regional-only deployment SKUs' (
  $deploymentSkuValues.Count -eq 15 -and
  (@($deploymentSkuValues | Where-Object { $_ -cne 'DataZoneStandard' }).Count -eq 0)
) "deploymentSkuValues=$($deploymentSkuValues -join ',')" 'DU-11'
...
$approvedModels = [ordered]@{
  luna = @{ name = 'gpt-5.6-luna'; version = '2026-07-09' }
  terra = @{ name = 'gpt-5.6-terra'; version = '2026-07-09' }
  sol = @{ name = 'gpt-5.6-sol'; version = '2026-07-09' }
  embedding = @{ name = 'text-embedding-3-large' }
}
$portfolioFailures = [Collections.Generic.List[string]]::new()
$portfolioFlag = Get-ObjectValue $params 'modelPortfolioDeploymentEnabled'
$portfolioFlagIsBoolean = $portfolioFlag -is [bool]
$portfolioEnabled = $portfolioFlagIsBoolean -and $portfolioFlag
$modelMap = Get-ObjectValue $params 'modelNameVersionAndQuota'

if (-not $portfolioFlagIsBoolean) {
  $portfolioFailures.Add('modelPortfolioDeploymentEnabled must be explicitly supplied as a Boolean')
}

$actualModelKeys = @(Get-ObjectKeys $modelMap | Sort-Object)
$expectedModelKeys = @($approvedModels.Keys | Sort-Object)
if (
  $actualModelKeys.Count -ne $expectedModelKeys.Count -or
  ($actualModelKeys -join ',') -cne ($expectedModelKeys -join ',')
) {
  $portfolioFailures.Add('modelNameVersionAndQuota must contain exactly embedding, luna, sol and terra')
}

foreach($modelKey in $approvedModels.Keys) {
  $expected = $approvedModels[$modelKey]
  $actual = Get-ObjectValue $modelMap $modelKey
  if ($null -eq $actual) {
    $portfolioFailures.Add("missing model map key $modelKey")
    continue
  }
  if ([string]$actual.name -cne [string]$expected.name) {
    $portfolioFailures.Add("model name mismatch for $modelKey")
  }
  if ($expected.ContainsKey('version') -and [string]$actual.version -cne [string]$expected.version) {
    $portfolioFailures.Add("model version mismatch for $modelKey")
  }
  $capacity = Get-IntOrDefault $actual.capacity -Default -1
  if ($portfolioEnabled -and $capacity -le 0) {
    $portfolioFailures.Add("positive owner-supplied capacity required for enabled model $modelKey")
  }
  elseif (-not $portfolioEnabled -and $capacity -lt 0) {
    $portfolioFailures.Add("capacity must be zero or positive for disabled model $modelKey")
  }
}

foreach($envName in @('dev','tst','prd')) {
  $deployments = @(To-Enumerable (Get-ObjectValue $params "du11.aiByEnvironment.$envName.deployments"))
  $deploymentKeys = @($deployments | ForEach-Object { [string]$_.modelKey })
  foreach($modelKey in $approvedModels.Keys) {
    if (@($deploymentKeys | Where-Object { $_ -ceq $modelKey }).Count -ne 1) {
      $portfolioFailures.Add("$envName must contain exactly one $modelKey deployment")
    }
  }
  if ($deployments.Count -ne $approvedModels.Count) {
    $portfolioFailures.Add("$envName must contain exactly four approved deployments")
  }
  foreach($dep in $deployments) {
    if ([string]::IsNullOrWhiteSpace([string]$dep.name)) {
      $portfolioFailures.Add("$envName deployment name is empty")
    }
    if ([string]$dep.versionUpgradeOption -cne 'NoAutoUpgrade') {
      $portfolioFailures.Add("$envName $([string]$dep.modelKey) versionUpgradeOption is not NoAutoUpgrade")
    }
    $fineTuningValue = if ($dep.PSObject.Properties.Name -contains 'fineTuningEnabled') {
      $dep.fineTuningEnabled
    }
    else {
      $null
    }
    if ($fineTuningValue -isnot [bool] -or $fineTuningValue -ne $false) {
      $portfolioFailures.Add("$envName $([string]$dep.modelKey) fine-tuning must be disabled")
    }
  }
}
if ($portfolioEnabled) {
  $evidenceByEnvironment = Get-ObjectValue $params 'modelCapabilityAndQuotaEvidenceByEnvironment'
  $expectedEvidenceEnvironmentKeys = @('dev','tst','prd')
  if (-not (Test-ExactObjectKeys $evidenceByEnvironment $expectedEvidenceEnvironmentKeys)) {
    $portfolioFailures.Add('enabled model portfolio requires exactly dev, tst and prd capability/quota evidence records')
  }

  foreach($envName in $expectedEvidenceEnvironmentKeys) {
    $evidence = Get-ObjectValue $evidenceByEnvironment $envName
    $expectedEvidenceKeys = @(
      ...
      'quotaEvidenceSha256',
      'target',
      'portfolioBindingSha256'
    )
    if (-not (Test-ExactObjectKeys $evidence $expectedEvidenceKeys)) {
      $portfolioFailures.Add("$envName capability/quota evidence record has an invalid shape")
      continue
    }

    $target = Get-ObjectValue $evidence 'target'
    $expectedTargetKeys = @('environment','subscriptionId','resourceGroupName','location','accountName')
    if (-not (Test-ExactObjectKeys $target $expectedTargetKeys)) {
      $portfolioFailures.Add("$envName capability/quota evidence target has an invalid shape")
      continue
    }

    $capabilityEvidenceId = Get-ObjectValue $evidence 'capabilityEvidenceId'
    $capabilityEvidenceHash = Get-ObjectValue $evidence 'capabilityEvidenceSha256'
    $quotaEvidenceId = Get-ObjectValue $evidence 'quotaEvidenceId'
    $quotaEvidenceHash = Get-ObjectValue $evidence 'quotaEvidenceSha256'
    $envToken = $envName.ToUpperInvariant()
    if (
      -not (Test-StrictEvidenceString $capabilityEvidenceId) -or
      $capabilityEvidenceId -cnotmatch "\ASTRATTON-AI-CAPABILITY-$envToken-[A-Z0-9]+(?:-[A-Z0-9]+)*\z" -or
      -not (Test-Sha256Hex $capabilityEvidenceHash)
    ) {
      $portfolioFailures.Add("$envName capability evidence requires a typed target-bound ID and lowercase SHA-256 hash")
    }
    if (
      -not (Test-StrictEvidenceString $quotaEvidenceId) -or
      $quotaEvidenceId -cnotmatch "\ASTRATTON-AI-QUOTA-$envToken-[A-Z0-9]+(?:-[A-Z0-9]+)*\z" -or
      -not (Test-Sha256Hex $quotaEvidenceHash)
    ) {
      $portfolioFailures.Add("$envName quota evidence requires a typed target-bound ID and lowercase SHA-256 hash")
    }
    if (-not (Test-StrictEvidenceString (Get-ObjectValue $evidence 'approvalState')) -or [string](Get-ObjectValue $evidence 'approvalState') -cne 'APPROVED') {
      $portfolioFailures.Add("$envName capability/quota evidence approvalState must be APPROVED")
    }

    $expectedSubscriptionId = if ($envName -eq 'prd') {
      Get-ObjectValue $params 'strattonProductionSubscriptionId'
    }
    else {
      Get-ObjectValue $params 'strattonNonproductionSubscriptionId'
    }
    $configuredSubscriptionId = Get-ObjectValue $params "du11.subscriptionIdByEnvironment.$envName"
    $configuredResourceGroupName = Get-ObjectValue $params "du11.aiResourceGroupByEnvironment.$envName"
    $configuredLocation = Get-ObjectValue $params "du11.locationByEnvironment.$envName"
    $configuredAccountName = Get-ObjectValue $params "du11.aiByEnvironment.$envName.openAi.accountName"
    $targetMatchesConfiguration = (
      (Test-StrictNonEmptyString $expectedSubscriptionId) -and
      (Test-StrictNonEmptyString $configuredSubscriptionId) -and
      (Test-StrictNonEmptyString $configuredResourceGroupName) -and
      (Test-StrictNonEmptyString $configuredLocation) -and
      (Test-StrictNonEmptyString $configuredAccountName) -and
      (Test-StrictEvidenceString (Get-ObjectValue $target 'environment')) -and
      (Test-StrictEvidenceString (Get-ObjectValue $target 'subscriptionId')) -and
      (Test-StrictEvidenceString (Get-ObjectValue $target 'resourceGroupName')) -and
      (Test-StrictEvidenceString (Get-ObjectValue $target 'location')) -and
      (Test-StrictEvidenceString (Get-ObjectValue $target 'accountName')) -and
      ([string]$configuredSubscriptionId -ceq [string]$expectedSubscriptionId) -and
      ([string](Get-ObjectValue $target 'environment') -ceq $envName) -and
      ([string](Get-ObjectValue $target 'subscriptionId') -ceq [string]$configuredSubscriptionId) -and
      ([string](Get-ObjectValue $target 'resourceGroupName') -ceq [string]$configuredResourceGroupName) -and
      ([string](Get-ObjectValue $target 'location') -ceq [string]$configuredLocation) -and
      ([string](Get-ObjectValue $target 'accountName') -ceq [string]$configuredAccountName)
    )
    if (-not $targetMatchesConfiguration) {
      $portfolioFailures.Add("$envName capability/quota evidence target does not match the exact DU-11 subscription, resource group, location and account")
    }

    $bindingLines = [Collections.Generic.List[string]]::new()
    $bindingLines.Add('STRATTON-AI-CAPABILITY-QUOTA-BINDING-V1')
    $bindingLines.Add("environment=$envName")
    $bindingLines.Add("subscriptionId=$([string]$configuredSubscriptionId)")
    $bindingLines.Add("resourceGroupName=$([string]$configuredResourceGroupName)")
    $bindingLines.Add("location=$([string]$configuredLocation)")
    $bindingLines.Add("accountName=$([string]$configuredAccountName)")
    $bindingLines.Add("approvalState=$([string](Get-ObjectValue $evidence 'approvalState'))")
    $bindingLines.Add("capabilityEvidenceId=$([string]$capabilityEvidenceId)")
    $bindingLines.Add("capabilityEvidenceSha256=$([string]$capabilityEvidenceHash)")
    $bindingLines.Add("quotaEvidenceId=$([string]$quotaEvidenceId)")
    $bindingLines.Add("quotaEvidenceSha256=$([string]$quotaEvidenceHash)")
    $deployments = @(To-Enumerable (Get-ObjectValue $params "du11.aiByEnvironment.$envName.deployments"))
    foreach($modelKey in $approvedModels.Keys) {
      $deployment = @($deployments | Where-Object { [string]$_.modelKey -ceq $modelKey } | Select-Object -First 1)
      $actualModel = Get-ObjectValue $modelMap $modelKey
      $fineTuningValue = if ($deployment.Count -eq 1) { Get-ObjectValue $deployment[0] 'fineTuningEnabled' } else { $null }
      $fineTuningText = if ($fineTuningValue -is [bool]) { $fineTuningValue.ToString().ToLowerInvariant() } else { [string]$fineTuningValue }
      $deploymentName = if ($deployment.Count -eq 1) { [string](Get-ObjectValue $deployment[0] 'name') } else { '' }
      $deploymentSkuName = if ($deployment.Count -eq 1) { [string](Get-ObjectValue $deployment[0] 'skuName') } else { '' }
      $deploymentVersionUpgradeOption = if ($deployment.Count -eq 1) { [string](Get-ObjectValue $deployment[0] 'versionUpgradeOption') } else { '' }
      $bindingLines.Add("modelKey=$modelKey")
      $bindingLines.Add("deploymentName=$deploymentName")
      $bindingLines.Add("modelName=$([string](Get-ObjectValue $actualModel 'name'))")
      $bindingLines.Add("modelVersion=$([string](Get-ObjectValue $actualModel 'version'))")
      $bindingLines.Add("capacity=$(Get-IntOrDefault (Get-ObjectValue $actualModel 'capacity') -Default -1)")
      $bindingLines.Add("skuName=$deploymentSkuName")
      $bindingLines.Add("versionUpgradeOption=$deploymentVersionUpgradeOption")
      $bindingLines.Add("fineTuningEnabled=$fineTuningText")
    }
    $expectedPortfolioBindingHash = Get-Sha256Hex ($bindingLines -join "`n")
    $suppliedPortfolioBindingHash = Get-ObjectValue $evidence 'portfolioBindingSha256'
    if (-not (Test-Sha256Hex $suppliedPortfolioBindingHash) -or $suppliedPortfolioBindingHash -cne $expectedPortfolioBindingHash) {
      $portfolioFailures.Add("$envName capability/quota evidence portfolio binding hash is stale, malformed or unbound")
    }
  }
}

Add-Assertion 'ASR-20' 'AI fine-tuning is disabled; approved model names, versions, NoAutoUpgrade and target-bound capability/quota evidence are pinned' (
  $portfolioFailures.Count -eq 0
) "modelPortfolioDeploymentEnabled=$portfolioFlag failures=$($portfolioFailures -join '; ')" 'DU-11'
