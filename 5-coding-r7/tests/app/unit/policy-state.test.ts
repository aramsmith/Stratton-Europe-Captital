...
function baseEvidence(): TransitionPolicyEvidence {
  return {
    ...
    modelResidencyEvidencePresent: true,
...
test("policy allows analysis when model residency evidence is present", () => {
  const decision = evaluateTransitionPolicy({
    tenantId: "tenant",
    caseId: "case",
...
test("policy denies analysis when model residency evidence is missing", () => {
  const decision = evaluateTransitionPolicy({
    tenantId: "tenant",
    caseId: "case",
    currentStatus: "EVIDENCE_ADMITTED",
    event: "REQUEST_ANALYSIS",
    evidence: { ...baseEvidence(), modelResidencyEvidencePresent: false }
  });
  assert.equal(decision.allowed, false);
  assert.deepEqual(decision.denialReasons, ["MODEL_REGION_EVIDENCE_MISSING"]);
});
