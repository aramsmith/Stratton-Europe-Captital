...
const { DefaultAzureCredential } = requireFromApp("@azure/identity");
...
const defaultAnalysisRoute = {
  modelDeploymentId: "model-a",
  deploymentResidencyEvidenceId: "region",
...
test("Document Intelligence uses the prebuilt-layout 2024-11-30 analyze URL", async () => {
  const originalGetToken = DefaultAzureCredential.prototype.getToken;
  const originalFetch = globalThis.fetch;
  const requestUrls: string[] = [];
  DefaultAzureCredential.prototype.getToken = async () => ({
    token: "test-token",
    expiresOnTimestamp: 4_102_444_800_000
  });
  globalThis.fetch = async (input: Parameters<typeof fetch>[0]) => {
    ...
    if (requestUrls.length === 1) {
      return new Response("", {
        status: 202,
        headers: { "operation-location": "https://operation.example.test/result" }
      });
    }
    return new Response(JSON.stringify({ status: "succeeded", analyzeResult: { content: "claim" } }), {
      status: 200
    });
  };

  try {
    const provider = new AzureDocumentIntelligenceProvider("https://document.example.test///");
    const claims = await provider.extractClaims("https://document.example.test/source.pdf");

    assert.equal(claims.length, 1);
    assert.deepEqual(requestUrls, [
      "https://document.example.test/documentintelligence/documentModels/prebuilt-layout:analyze?api-version=2024-11-30",
      "https://operation.example.test/result"
    ]);
  } finally {
    DefaultAzureCredential.prototype.getToken = originalGetToken;
...
class RecordingAnalysisProvider implements AnalysisProvider {
  ...
  public async runDraftOnlyAnalysis(
    input: Parameters<AnalysisProvider["runDraftOnlyAnalysis"]>[0]
  ): Promise<{
