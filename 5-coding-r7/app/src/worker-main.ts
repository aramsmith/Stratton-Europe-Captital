...
async function run(): Promise<void> {
  ...
  const { FileIdempotencyStore, InMemoryIdempotencyStore, SqlIdempotencyStore } =
    await import("./idempotency-store.js");
  const {
  ...
  } = await import("./provider-adapters.js");
  const { AzureServiceBusFactory, InMemoryQueueRouter } =
    await import("./queue-adapters.js");
  const { AzureSqlExecutor } = await import("./sql-client.js");
  const { AzureBlobReferenceProvider, InMemoryBlobReferenceProvider } =
    await import("./source-connector.js");
  const { InMemoryWorkloadRepository, SqlWorkloadRepository } =
    await import("./workload-repository.js");
  const { WorkerRuntime } = await import("./worker-runtime.js");
  ...
  if (mode === "production") {
    ...
    const runtime = new WorkerRuntime({
      ...
      modelRoutingPolicyVersion: config.modelRoutingPolicyVersion,
