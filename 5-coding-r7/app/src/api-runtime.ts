...
function parseModelTaskClass(body: Record<string, unknown>): ModelTaskClass {
  const value = requireString(body, "taskClass");
  switch (value) {
...
function parseModelEscalationReason(
  body: Record<string, unknown>
): ModelEscalationReason | undefined {
  const value = body.escalationReason;
  if (value === undefined) {
    return undefined;
  }
...
function assertModelRouteAuthority(
  principal: AuthenticatedPrincipal,
  escalationReason: ModelEscalationReason | undefined,
  authorityErrorCode: "ESCALATION_AUTHORITY_REQUIRED"
): void {
  if (escalationReason === undefined) {
    if (principal.roles.includes("DealContributor")) {
      return;
    }
    throw new HttpError(403, authorityErrorCode);
  }
  switch (escalationReason) {
    ...
    case "CONFLICTING_MATERIAL_EVIDENCE":
    case "HIGH_RISK_SPECIALIST_CONCLUSION":
      if (principal.roles.includes("ValidationProducer")) {
        return;
      }
      throw new HttpError(403, authorityErrorCode);
    case "AUTHORISED_HUMAN_REQUEST":
      if (
        principal.isHuman &&
        ["DealReviewer", "LegalApprover", "ComplianceApprover"].some((role) =>
...
async function buildPolicyEvidence(
  ...
  return {
    ...
    modelResidencyEvidencePresent:
      analysis !== undefined &&
      analysis.modelDeploymentId ===
        config.modelRouteDeployments[analysis.modelTier].deploymentId &&
      analysis.deploymentResidencyEvidenceId ===
        config.modelRouteDeployments[analysis.modelTier].residencyEvidenceId,
...
function errorMessage(code: ErrorPayload["code"]): string {
  const map: Readonly<Record<ErrorPayload["code"], string>> = {
    ...
    ESCALATION_AUTHORITY_REQUIRED: "The principal lacks authority for the requested route.",
  ...
  return [
    ...
    },
    {
      ...
      handler: async ({ body, principal, correlationId, request }, match) => {
        ...
        const taskClass = parseModelTaskClass(body);
        const escalationReason = parseModelEscalationReason(body);
        assertModelRouteAuthority(
          principal,
          escalationReason,
          "ESCALATION_AUTHORITY_REQUIRED"
        );
        const routeDecision = selectModelRoute(
          escalationReason === undefined
            ? { taskClass }
            : { taskClass, escalationReason },
          config.modelRouteDeployments
        );
        ...
        const promptTemplateVersion = requireString(body, "promptTemplateVersion");
        const run: AnalysisRunRecord = {
          ...
          modelDeploymentId: routeDecision.deploymentId,
          modelProviderEvidenceId: config.modelProviderEvidenceId,
          deploymentResidencyEvidenceId: routeDecision.deploymentResidencyEvidenceId,
          ...
          inputManifestHash: createHash("sha256")
            .update(
              canonical({
                ...
                modelRouteReason: routeDecision.modelRouteReason,
                modelReasoningEffort: routeDecision.reasoningEffort,
                modelRoutingPolicyVersion: routeDecision.modelRoutingPolicyVersion,
                modelDeploymentId: routeDecision.deploymentId,
                deploymentResidencyEvidenceId:
                  routeDecision.deploymentResidencyEvidenceId,
                modelName: routeDecision.modelName,
                modelVersion: routeDecision.modelVersion
              })
            )
            .digest("hex"),
          modelTaskClass: taskClass,
          modelTier: routeDecision.tier,
          modelRouteReason: routeDecision.modelRouteReason,
          modelReasoningEffort: routeDecision.reasoningEffort,
          modelRoutingPolicyVersion: routeDecision.modelRoutingPolicyVersion,
          modelName: routeDecision.modelName,
          modelVersion: routeDecision.modelVersion,
          modelValidationStatus: routeDecision.modelValidationStatus,
          status: "QUEUED",
        ...
        const context: TransitionContext = {
          ...
          evidence: {
            ...
            modelResidencyEvidencePresent:
              routeDecision.deploymentId ===
                config.modelRouteDeployments[routeDecision.tier].deploymentId &&
              routeDecision.deploymentResidencyEvidenceId ===
                config.modelRouteDeployments[routeDecision.tier].residencyEvidenceId,
        ...
        return {
          ...
          body: {
            ...
            modelRouteReason: run.modelRouteReason,
            modelRoutingPolicyVersion: run.modelRoutingPolicyVersion
          }
        };
      }
    },
