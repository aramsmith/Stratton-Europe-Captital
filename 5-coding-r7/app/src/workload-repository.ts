...
export class InMemoryWorkloadRepository implements WorkloadRepository {
  ...
  public async updateAnalysisRunObservation(
    tenantId: string,
    caseId: string,
    analysisRunId: string,
    observation: AnalysisRunObservation
  ): Promise<void> {
    const key = key3(tenantId, caseId, analysisRunId);
    const current = this.analysisRuns.get(key);
    if (!current) {
    ...
    this.analysisRuns.set(key, {
      ...current,
      modelValidationStatus: observation.modelValidationStatus,
      ...(observation.modelLatencyMilliseconds !== undefined
        ? { modelLatencyMilliseconds: observation.modelLatencyMilliseconds }
        : {}),
      ...(observation.modelInputTokens !== undefined
        ? { modelInputTokens: observation.modelInputTokens }
        : {}),
      ...(observation.modelOutputTokens !== undefined
        ? { modelOutputTokens: observation.modelOutputTokens }
        : {}),
      ...(observation.modelObservedCostUsd !== undefined
        ? { modelObservedCostUsd: observation.modelObservedCostUsd }
        : {})
    });
  }
...
BEGIN
  INSERT INTO dbo.analysis_runs (
    ...
    model_provider_evidence_id, deployment_residency_evidence_id, prompt_governance_evidence_id,
    prompt_template_version, policy_version, input_manifest_hash, model_task_class, model_tier,
    model_route_reason, model_reasoning_effort, model_routing_policy_version, model_name,
    model_version, model_validation_status, model_latency_milliseconds, model_input_tokens,
    model_output_tokens, model_observed_cost_usd, status, output_kind, unsupported_claims,
...
END;
      `,
      {
        ...
        deployment_residency_evidence_id: record.deploymentResidencyEvidenceId,
        ...
        model_task_class: record.modelTaskClass,
        model_tier: record.modelTier,
        model_route_reason: record.modelRouteReason,
        model_reasoning_effort: record.modelReasoningEffort,
        model_routing_policy_version: record.modelRoutingPolicyVersion,
        model_name: record.modelName,
        model_version: record.modelVersion,
        model_validation_status: record.modelValidationStatus,
        model_latency_milliseconds: record.modelLatencyMilliseconds,
        model_input_tokens: record.modelInputTokens,
        model_output_tokens: record.modelOutputTokens,
        model_observed_cost_usd: record.modelObservedCostUsd,
        status: record.status,
  ...
  public async getAnalysisRun(
    ...
    const rows = await this.executor.queryMany<{
      ...
      deployment_residency_evidence_id: string;
      prompt_governance_evidence_id: string;
      prompt_template_version: string;
      policy_version: string;
      input_manifest_hash: string;
      model_task_class: AnalysisRunRecord["modelTaskClass"];
      model_tier: AnalysisRunRecord["modelTier"];
      model_route_reason: string;
      model_reasoning_effort: AnalysisRunRecord["modelReasoningEffort"];
      model_routing_policy_version: AnalysisRunRecord["modelRoutingPolicyVersion"];
      model_name: AnalysisRunRecord["modelName"];
      model_version: AnalysisRunRecord["modelVersion"];
      model_validation_status: AnalysisRunRecord["modelValidationStatus"];
      model_latency_milliseconds: number | null;
      model_input_tokens: number | null;
      model_output_tokens: number | null;
      model_observed_cost_usd: number | null;
      status: AnalysisRunRecord["status"];
...
WHERE tenant_id=@tenant_id AND case_id=@case_id AND analysis_run_id=@analysis_run_id;
    ...
    if (
      row.model_latency_milliseconds !== null &&
      row.model_input_tokens !== null &&
      row.model_output_tokens !== null &&
      row.model_observed_cost_usd !== null
    ) {
      return {
        tenantId: row.tenant_id,
        caseId: row.case_id,
        ...
        deploymentResidencyEvidenceId: row.deployment_residency_evidence_id,
        ...
        modelRoutingPolicyVersion: row.model_routing_policy_version,
        modelName: row.model_name,
        modelVersion: row.model_version,
        modelValidationStatus: row.model_validation_status,
        modelLatencyMilliseconds: row.model_latency_milliseconds,
        modelInputTokens: row.model_input_tokens,
        modelOutputTokens: row.model_output_tokens,
        modelObservedCostUsd: row.model_observed_cost_usd,
        status: row.status,
...
WHERE tenant_id=@tenant_id AND case_id=@case_id AND analysis_run_id=@analysis_run_id;
  ...
  public async updateAnalysisRunObservation(
    ...
    await this.executor.execute(
      `
UPDATE dbo.analysis_runs
SET model_validation_status=@model_validation_status,
    model_latency_milliseconds=COALESCE(@model_latency_milliseconds, model_latency_milliseconds),
    model_input_tokens=COALESCE(@model_input_tokens, model_input_tokens),
    model_output_tokens=COALESCE(@model_output_tokens, model_output_tokens),
    model_observed_cost_usd=COALESCE(@model_observed_cost_usd, model_observed_cost_usd),
    updated_at=SYSUTCDATETIME()
WHERE tenant_id=@tenant_id AND case_id=@case_id AND analysis_run_id=@analysis_run_id;
      `,
      {
        ...
        case_id: caseId,
        analysis_run_id: analysisRunId,
        model_validation_status: observation.modelValidationStatus,
        model_latency_milliseconds: observation.modelLatencyMilliseconds ?? null,
        model_input_tokens: observation.modelInputTokens ?? null,
        model_output_tokens: observation.modelOutputTokens ?? null,
        model_observed_cost_usd: observation.modelObservedCostUsd ?? null
      },
      { context: { tenantId, caseId } }
    );
  }
