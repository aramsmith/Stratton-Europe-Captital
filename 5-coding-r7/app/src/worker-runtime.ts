...
import type { ModelRouteDeployments } from "./model-routing-policy.js";
...
function analysisAuthorityBlockedReason(
  sourceStatus: "DISABLED" | "ACTIVE" | "SUSPENDED" | undefined,
  licence: { readonly expiresAtIso: string; readonly aiAnalysisAllowed: boolean } | undefined,
  nowEpochMs: number
): string | undefined {
  if (sourceStatus === undefined) {
    return "ANALYSIS_AUTHORITY_SOURCE_MISSING";
  }
  if (sourceStatus !== "ACTIVE") {
    return "ANALYSIS_AUTHORITY_SOURCE_NOT_ACTIVE";
  }
  if (!licence) {
    return "ANALYSIS_AUTHORITY_LICENCE_MISSING";
  }
  const expiresAtEpochMs = Date.parse(licence.expiresAtIso);
  if (!Number.isFinite(expiresAtEpochMs)) {
    return "ANALYSIS_AUTHORITY_LICENCE_EXPIRY_INVALID";
  }
  if (expiresAtEpochMs <= nowEpochMs) {
    return "ANALYSIS_AUTHORITY_LICENCE_EXPIRED";
  }
  if (!licence.aiAnalysisAllowed) {
    return "ANALYSIS_AUTHORITY_AI_ANALYSIS_NOT_ALLOWED";
  }
  return undefined;
}

function observedInteger(value: number | undefined, field: string): number | undefined {
  if (
    value !== undefined &&
    (!Number.isSafeInteger(value) || value < 0)
  ) {
    throw new Error(`INVALID_ANALYSIS_PROVIDER_OBSERVATION:${field}`);
  }
  return value;
}

function observedCost(value: number | undefined): number | undefined {
  if (value !== undefined && (!Number.isFinite(value) || value < 0)) {
    throw new Error("INVALID_ANALYSIS_PROVIDER_OBSERVATION:modelObservedCostUsd");
  }
  return value;
}

interface ValidatedProviderTelemetry {
  readonly modelLatencyMilliseconds?: number;
  readonly modelInputTokens?: number;
  readonly modelOutputTokens?: number;
  readonly modelObservedCostUsd?: number;
}

function validateProviderResultSchema(providerResult: AnalysisProviderResult): boolean {
  return (
    providerResult.outputReference.trim().length > 0 &&
    Array.isArray(providerResult.claims) &&
    providerResult.claims.every(
      (claim) =>
        claim.claimId.trim().length > 0 &&
        claim.claimTextReference.trim().length > 0 &&
        (claim.severity === "CRITICAL" || claim.severity === "NON_CRITICAL") &&
        typeof claim.isMaterial === "boolean" &&
        (claim.unsupportedReason === undefined ||
          claim.unsupportedReason.trim().length > 0)
    ) &&
    Array.isArray(providerResult.citations) &&
    providerResult.citations.every(
      (citation) =>
        citation.citationId.trim().length > 0 &&
        citation.claimId.trim().length > 0 &&
        citation.evidenceId.trim().length > 0 &&
        citation.evidenceVersionId.trim().length > 0 &&
        citation.locator.trim().length > 0 &&
        typeof citation.accessibleAtReview === "boolean"
    )
  );
}
...
): ValidatedProviderTelemetry {
  const modelLatencyMilliseconds = observedInteger(
    providerResult.modelLatencyMilliseconds,
    "modelLatencyMilliseconds"
  );
  const modelInputTokens = observedInteger(
    providerResult.modelInputTokens,
    "modelInputTokens"
  );
  const modelOutputTokens = observedInteger(
    providerResult.modelOutputTokens,
    "modelOutputTokens"
  );
  const modelObservedCostUsd = observedCost(providerResult.modelObservedCostUsd);
  return {
    ...(modelLatencyMilliseconds !== undefined ? { modelLatencyMilliseconds } : {}),
    ...(modelInputTokens !== undefined ? { modelInputTokens } : {}),
    ...(modelOutputTokens !== undefined ? { modelOutputTokens } : {}),
    ...(modelObservedCostUsd !== undefined ? { modelObservedCostUsd } : {})
  };
}

function buildAnalysisRunObservation(
  telemetry: ValidatedProviderTelemetry,
  modelValidationStatus: "PASS" | "FAIL"
): AnalysisRunObservation {
  return {
    modelValidationStatus,
    ...(telemetry.modelLatencyMilliseconds !== undefined
      ? { modelLatencyMilliseconds: telemetry.modelLatencyMilliseconds }
      : {}),
    ...(telemetry.modelInputTokens !== undefined
      ? { modelInputTokens: telemetry.modelInputTokens }
      : {}),
    ...(telemetry.modelOutputTokens !== undefined
      ? { modelOutputTokens: telemetry.modelOutputTokens }
      : {}),
    ...(telemetry.modelObservedCostUsd !== undefined
      ? { modelObservedCostUsd: telemetry.modelObservedCostUsd }
      : {})
  };
}
  ...
  private async handleAnalysis(message: QueueMessage): Promise<void> {
    ...
    const state = await this.config.repository.withCaseTransaction(
      ...
      async (repository) => {
        ...
        return { run, caseRecord, evidence, approvedDeal, approvedJurisdiction };
      }
    );
    if ("blockedReason" in state) {
      throw new Error(state.blockedReason);
    }
    const authority = await this.config.repository.withCaseTransaction(
      message.tenantId,
      message.caseId,
      async (repository) => {
        const source = await repository.getSource(
          message.tenantId,
          message.caseId,
          state.evidence.sourceId
        );
        const licence = await repository.getLatestExternalLicenceDecision(
          message.tenantId,
          message.caseId,
          state.evidence.sourceId
        );
        const blockedReason = analysisAuthorityBlockedReason(
          source?.status,
          licence,
          Date.now()
        );
        if (blockedReason) {
          ...
          await repository.appendAuditEvent({
            ...
            sourceEventId: auditSourceEventId(
              message,
              `ANALYSIS_BLOCKED_AUTHORITY_${blockedReason}`
            ),
            actorId: "worker-analysis",
            action: "ANALYSIS_BLOCKED",
            subjectId: analysisRunId,
            correlationId: message.correlationId,
            outcome: "DENY",
            payloadReference: `[REDACTED]:${blockedReason}`
          });
          return { blockedReason };
        }
    ...
    if ("blockedReason" in authority) {
      throw new Error(authority.blockedReason);
    }
    const currentSource = authority.source;
    const currentLicence = authority.licence;
    if (!currentSource || !currentLicence) {
      throw new Error("ANALYSIS_AUTHORITY_RECHECK_INCONSISTENT");
    }
    let providerResult: Awaited<ReturnType<AnalysisProvider["runDraftOnlyAnalysis"]>>;
    let providerTelemetry: ValidatedProviderTelemetry;
    try {
      const configuredDeployment =
        this.config.modelRouteDeployments[state.run.modelTier];
      if (
        state.run.modelDeploymentId !== configuredDeployment.deploymentId ||
        state.run.deploymentResidencyEvidenceId !==
          configuredDeployment.residencyEvidenceId ||
        state.run.modelName !== configuredDeployment.modelName ||
        state.run.modelVersion !== configuredDeployment.modelVersion ||
        state.run.modelRoutingPolicyVersion !==
          this.config.modelRoutingPolicyVersion
      ) {
        throw new Error("MODEL_ROUTE_CONFIGURATION_DRIFT");
      }
      providerResult = await this.config.analysisProvider.runDraftOnlyAnalysis({
        analysisRunId,
        payloadReference: state.evidence.payloadReference,
        modelDeploymentId:
          this.config.modelRouteDeployments[state.run.modelTier].deploymentId,
        deploymentResidencyEvidenceId:
          this.config.modelRouteDeployments[state.run.modelTier].residencyEvidenceId,
        modelTaskClass: state.run.modelTaskClass,
        modelTier: state.run.modelTier,
        modelRouteReason: state.run.modelRouteReason,
        modelReasoningEffort: state.run.modelReasoningEffort,
        modelRoutingPolicyVersion: this.config.modelRoutingPolicyVersion,
        modelName:
          this.config.modelRouteDeployments[state.run.modelTier].modelName,
        modelVersion:
          this.config.modelRouteDeployments[state.run.modelTier].modelVersion,
        promptTemplateVersion: state.run.promptTemplateVersion
      });
      if (!validateProviderResultSchema(providerResult)) {
        throw new Error("ANALYSIS_PROVIDER_SCHEMA_VALIDATION_FAILED");
      }
      providerTelemetry = validateProviderTelemetry(providerResult);
    } catch (error) {
      await this.config.repository.withCaseTransaction(message.tenantId, message.caseId, async (repository) => {
        await repository.updateAnalysisRunStatus(
          message.tenantId,
    ...
    const denialMarker = await this.config.repository.withCaseTransaction(
      ...
      const transition: TransitionContext = {
        ...
        evidence: {
          ...
          sourceActive: currentSource.status === "ACTIVE",
          permissionScopeAllowed: currentLicence.purposeApproved,
          purposeOfUseAllowed: currentLicence.purposeApproved,
          privacyLawfulBasisPresent: currentLicence.privacyApproved,
          externalDataLicencePresent: true,
          externalDataLicenceCompatible: currentLicence.licenceCompatible,
          aiRetrievalAllowed: currentLicence.aiRetrievalAllowed,
          aiAnalysisAllowed: currentLicence.aiAnalysisAllowed,
          ...
          modelResidencyEvidencePresent:
            state.run.modelDeploymentId ===
              this.config.modelRouteDeployments[state.run.modelTier].deploymentId &&
            state.run.deploymentResidencyEvidenceId ===
              this.config.modelRouteDeployments[state.run.modelTier].residencyEvidenceId,
      ...
      const citationValidationPassed =
        assessment.totalClaimCount > 0 && assessment.allMaterialClaimsCited;
      const factualConsistencyValidationPassed =
        assessment.criticalUnsupportedClaimCount === 0;
      await repository.updateAnalysisRunObservation(
        message.tenantId,
        message.caseId,
        analysisRunId,
        buildAnalysisRunObservation(
          providerTelemetry,
          citationValidationPassed && factualConsistencyValidationPassed
            ? "PASS"
            : "FAIL"
        )
      );
