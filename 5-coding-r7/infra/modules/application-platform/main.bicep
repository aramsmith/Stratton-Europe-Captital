...
var approvedWorkerQueueNames = [
  'q-ingestion'
  'q-extraction'
  'q-indexing'
]
var activeWorkerJobs = filter(
  platform.workerJobs,
  workerJob => bool(workerJob.?deploymentEnabled ?? true) && contains(approvedWorkerQueueNames, workerJob.queueName)
)
  ...
  properties: {
    ...
    identityProviders: {
      azureActiveDirectory: {
        enabled: true
        registration: {
          clientId: platform.apiApp.entraAuthentication.clientId
          openIdIssuer: '${environment().authentication.loginEndpoint}${platform.apiApp.entraAuthentication.tenantId}/v2.0'
        }
        validation: {
          allowedAudiences: [
            platform.apiApp.entraAuthentication.allowedAudience
          ]
        }
      }
    }
...
resource workerJobs 'Microsoft.App/jobs@2024-03-01' = [for workerJob in activeWorkerJobs: {
  name: workerJob.name
  location: location
  tags: tags
  identity: {
...
resource acrPullAssignmentsWorker 'Microsoft.Authorization/roleAssignments@2022-04-01' = [for workerJob in activeWorkerJobs: {
...
output workerJobIds array = [for (workerJob, index) in activeWorkerJobs: workerJobs[index].id]
