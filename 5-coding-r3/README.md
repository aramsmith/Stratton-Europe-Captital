...
Phase 4 used `iac/...` as the logical release-package root. Model-plan revision 14 places this
revisioned Phase 5 candidate beneath `5-coding-r3/`, so the path mapping is:

| Approved logical path | Phase 5 path |
|---|---|
| `iac/infra/` | `5-coding-r3/infra/` |
| `iac/contracts/`, application and database work | `5-coding-r3/app/` |
| `iac/tests/` | `5-coding-r3/tests/` |
| `iac/validation/` | `5-coding-r3/validation/` |
| release and deployment instructions | `5-coding-r3/deploy/` and the root release manifest |
...
```powershell
pwsh -NoProfile -File 5-coding-r3\validation\Invoke-LocalValidation.ps1 -Scope All
```
