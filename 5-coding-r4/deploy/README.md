...
```powershell
pwsh -NoProfile -File .\5-coding-r4\validation\Invoke-LocalValidation.ps1 -Scope All
```
...
$compiledOutput = @(
  ...
  --file ".\5-coding-r4\infra\parameters\$Environment.bicepparam" `
  --stdout 2>&1 |
...
$preflight = @(
  '-NoProfile',
  '-File', (Join-Path $CaseRoot '5-coding-r4\tests\iac\Invoke-DeploymentPreflight.ps1'),
...
```powershell
$Template = Join-Path $CaseRoot '5-coding-r4\infra\main.bicep'
...
$AssuranceTemplate = Join-Path $CaseRoot '5-coding-r4\infra\assurance-main.bicep'
...
```powershell
& (Join-Path $CaseRoot '5-coding-r4\deploy\Invoke-AssuranceRetentionFinalization.ps1') `
