...
$revision15Path = Join-Path (Split-Path -Parent $root) `
    '0-coordination\stratton-model-plan-revision-15.json'
$revision15Hash = (Get-FileHash -LiteralPath $revision15Path -Algorithm SHA256).Hash.ToLowerInvariant()
if ($revision15Hash -cne '7ce3ff77d47384ceaf3e0515b3350578831b5a2fedfcbeba4e586399dc9f6fb4') {
    throw 'Model-plan revision 15 hash mismatch.'
}
$revision15 = Get-Content -Raw -LiteralPath $revision15Path | ConvertFrom-Json -Depth 30
if (
    [string]$revision15.planRevision -cne '15' -or
    [string]$revision15.revisionedCandidate.packageRoot -cne '5-coding-r4' -or
    [string]$revision15.revisionedCandidate.manifestPath -cne
        '5-coding-r4/stratton-phase-5-hashes.json'
) {
    throw 'Model-plan revision 15 does not bind the revisioned candidate package.'
}
