import type { WorkItemRecord } from "../../../app/src/types.js";
import { InMemoryWorkloadRepository } from "../../../app/src/workload-repository.js";

export async function createDemoAuthorityRepository(): Promise<InMemoryWorkloadRepository> {
  ...
  for (const subjectId of ["contributor-a", "reviewer-a"]) {
    await repository.grantCaseAccess({
      tenantId: "tenant-a",
      caseId: "case-1",
      subjectId,
      purpose: "DUE_DILIGENCE",
      role: subjectId === "contributor-a" ? "DealContributor" : "DealReviewer"
    });
  }
...
export default createDemoAuthorityRepository;
